describe("Terms of payment Validations Negative", () => {
  ...
  var cashDiscountPct1 = 2.0;
  var cashDiscountPeriod2 = 21;
  var cashDiscountPct2 = 1.0;
  var edi = 1;
  var changeState = 0;
  ...
  it("createTermsOfPaymentWithEmptyParameters", () => {
    ...
    cy.request({
      ...
      assert.equal(
        ...
        "Der Wert des Parameters 'ZLBD_CD' wurde nicht gefunden oder nicht eingegeben"
      );
  ...
  it("createTermsOfPaymentWithCashDiscountPeriod2GreaterThanCashDiscountPeriod2", () => {
    ...
    cy.request({
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.bfw.rw.fi.serv.DiscountPeriod2LessThanDiscountPeriod1Error"
      );
      //"The discount period2 must last longer than the discount period1"
      assert.equal(
        response.body.Messages[0]["Text"],
        "Die Skontofrist2 muss länger dauern als die Skontofrist1"
      );
  ...
  it("createTermsOfPaymentWithCashDiscountPeriod2GreaterThanNetDays", () => {
    ...
    cy.request({
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.bfw.rw.fi.serv.NetDaysAreLessThanDiscountPeriod2Error"
      );
      //The value of the parameter 'ZLBD_CD' was not found or not entered
      assert.equal(
        response.body.Messages[0]["Text"],
        "Die Nettofrist muss mindestenst so lange sein wie die Skontofrist2!"
      );
  ...
  it("createTermsOfPaymentCodeWithMoreThanThreeCharacters", () => {
    ...
    cy.request({
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.bfw.common.repository.EntityValidationMaxLengthError"
      );
      //"The value of the parameter 'ZLBD_CD' was not found or not entered"
      assert.equal(
        response.body.Messages[0]["Text"],
        "The contents of the property S_ZLBD.ZLBD_CD is longer than 3 characters!"
      );
  ...
  it("createTermsOfPaymentCodeWithExistingCode", () => {
    ...
    cy.request({
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.bfw.rw.fi.serv.TermsOfPaymentAlreadyExistsError"
      );
      //"Payment terms already exist!"
      assert.equal(
        response.body.Messages[0]["Text"],
        "Zahlungsbedingungen sind bereits vorhanden!"
      );
  ...
  it("createTermsOfPayment_BusinessLogic", () => {
    ...
    cy.request({
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.bfw.rw.fi.serv.DiscountDeductionNotSpecifiedError"
      );
      //If a discount period has been specified, a discount deduction must also be specified as a percentage!
      assert.equal(
        response.body.Messages[0]["Text"],
        "Wenn eine Skontofrist angegeben worden ist, muss auch ein Skontoabzug in Prozent angeben werden!"
      );

      assert.equal(
        response.body.Messages[1]["Id"],
        "rssoft.bfw.rw.fi.serv.NetDaysCannotBeEmptyError"
      );
      ...
      assert.equal(
        ...
        "Geben sie die Frist für die Netto-Tage an. (Frist der Netto-Tage muss länger bzw. gleich lang dauern wie Skontofrist1)"
      );

      assert.equal(
        response.body.Messages[2]["Id"],
        "rssoft.bfw.rw.fi.serv.NetDaysAreLessThanDiscountPeriod1Error"
      );
      //The net period must be at least as long as the discount period1!
      assert.equal(
        response.body.Messages[2]["Text"],
        "Die Nettofrist muss mindestenst so lange dauern wie Skontofrist1!"
      );
      expect(response.status).to.eq(200);
    });
  ...
  it("updateTermsOfPaymentWithInvalidStatusCode", () => {
    ...
    cy.request({
      ...
      body: {
        ...
        CashDiscountPct1: 2.0,
        NetDays: 30,
        CashDiscountPeriod2: 21,
        CashDiscountPct2: 1.0,
      ...
      assert.equal(
        response.body.Messages[0]["Id"],
        "rssoft.common.message.DoesNotExistsError"
      );
      //"Payment terms already exist!"
      assert.equal(
        response.body.Messages[0]["Text"],
        "Status 560 in ZLBD_STATUS does not exist!"
      );
